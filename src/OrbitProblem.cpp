...
double OrbitTransferObjective<T, Fun>::calculateDeltaV(const std::vector<double>& x)
{
    ...
    {
        //non-coplanar case
        std::cout << "Non-coplanar case not implemented yet." << std::endl;
        return 0;
    ...
    const Vector3& r1, const Vector3& r2, double tof, double mu, bool isLongWay)
{
    ...
    double g = r1_mag * r2_mag * std::sin(theta) / std::sqrt(mu * a);
    double g_dot = 1.0 - r1_mag / a * (1.0 - std::cos(theta));
    double f_dot = -std::sqrt(mu / a) * std::sin(theta) / (r1_mag * r2_mag) * r2_mag;
