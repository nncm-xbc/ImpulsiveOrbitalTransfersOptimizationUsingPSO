...
void TransferModel::generateTransferTrajectory() {
    ...
    {   
        ...
        float e_vec_length = glm::length(e_vec);
        if (e_vec_length > 1e-6f) {
            periapsis_dir = e_vec / e_vec_length;
            std::cout << "Periapsis direction1: " << periapsis_dir.x << ", " << periapsis_dir.y << ", " << periapsis_dir.z << std::endl;
        } else {
            periapsis_dir = glm::normalize(glm::cross(normal, glm::vec3(0.0f, 0.0f, 1.0f)));
            std::cout << "Periapsis direction2: " << periapsis_dir.x << ", " << periapsis_dir.y << ", " << periapsis_dir.z << std::endl;
            if (glm::length(periapsis_dir) < 1e-6f) {
                periapsis_dir = glm::normalize(glm::cross(normal, glm::vec3(1.0f, 0.0f, 0.0f)));
                std::cout << "Periapsis direction3: " << periapsis_dir.x << ", " << periapsis_dir.y << ", " << periapsis_dir.z << std::endl;
            }
        }
