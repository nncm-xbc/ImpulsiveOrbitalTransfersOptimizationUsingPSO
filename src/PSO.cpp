...
template <typename T, typename Fun> void PSO<T, Fun>::printResults() const {
  ...
  std::cout << std::endl;
  std::cout << "Best fitness value: " << swarm.getGlobalBestValue()
            << std::endl;
  std::cout << "Number of iterations: " << _convergenceHistory.size()
            << std::endl;
}
