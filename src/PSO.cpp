...
PSO<T, I, Fun, Obj>::PSO(const I &swarm_id, const I &max_iter, const T &tol,
  ...
  init(swarm_id, max_iter, tol, w, c1, c2, num_particles, fun, D,
       exact_solution);
}
template <typename T, typename I, typename Fun, typename Obj>
PSO<T, I, Fun, Obj>::PSO() {}
...
vector<T> PSO<T, I, Fun, Obj>::getGlobalBest() const {
  ...
  for (size_t i = 1; i < _particles.size(); ++i) {
    if (_particles[i].getBestValue() <
        _particles[best_part_id].getBestValue()) {
      best_part_id = i;
    }
  }
  //----------------------------------------------------------------------------------------------
  // Race condition when writing to best_part_id.
  // In order to avoid it, we use first have each thread compute their own
  // best_value Then I a critical section, one thread compares the best_value of
  // each thread and updates best_part_id (The overhead of parallelizing this is
  // not worth it)
  //----------------------------------------------------------------------------------------------
...
template <typename T, typename I, typename Fun, typename Obj>
void PSO<T, I, Fun, Obj>::init(const I &swarm_id, const I &max_iter,
                               const T &tol, const T &w, const T &c1,
                               const T &c2, const I &num_particles,
                               const Fun &fun, const I &D,
                               const vector<T> &exact_solution) {
...
#pragma omp parallel for
    for (int p = 0; p < _num_particles; ++p) {
      ...
      for (int d = 0; d < _D; ++d) {
        _particles[p].getVelocity()[d] =
            _w * _particles[p].getVelocity()[d] +
            _c1 * r1[d] *
                (_particles[p].getBestPosition()[d] -
                 _particles[p].getPosition()[d]) +
            _c2 * r2[d] * (_gbp[d] - _particles[p].getPosition()[d]);
      }
      for (int d = 0; d < _D; ++d) {
        _particles[p].getPosition()[d] =
            _particles[p].getPosition()[d] + _particles[p].getVelocity()[d];
      }
      localBest(_particles[p]);
    }
    ...
    if (_fun(gbp_new) < _fun(_gbp)) {
      ...
      _w = min(_w * 1.2,
               0.9); // correction, if you are doing good, you can go faster
      if (abs(_w - 0.9) < eps) {
        _w *= 0.95;
      } // if you are going too fast, slow down a little
    } else {
      _w = max(_w * 0.9,
               0.1); // correction, if you are doing bad, you should go slower
      if (abs(_w - 0.1) < eps) {
        _w *= 2;
      } // if you are going too slow, speed up a little
    }

    if (errorNorm(_gbp) < _tol) {
      cout << "\n Swarm " << _id << " --> Convergence achieved in " << it + 1
           << " iterations" << endl;
      return gbp_new.back();
    };
...
template class PSO<double, int, function<double(const vector<double> &)>,
                   Particle<double, function<double(const vector<double> &)>>>;
