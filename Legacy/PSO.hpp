...
template <typename T, typename I, typename Fun, typename Obj> class PSO {
public:
  // Constructor
  PSO(const I &swarm_id, const I &max_iter, const T &tol, const T &w,
      const T &c1, const T &c2, const I &num_particles, const Fun &fun,
      const I &D, const vector<T> &exact_solution);
  ...
  void setGlobalBest(
      const vector<T> &gbp); // set global best position after each iteration
  void setExactSolution(
      const vector<T> &exact_solution); // set exact solution of the problem
  ...
  // Member functions
  void localBest(
      Obj &particle) const; // auxiliary function to find the local best
                            // position of a particle within each iteration
  const T solve();          // PSO iteration
  void info(const string &fun_name) const; // print info about the PSO object
  const T errorNorm(
      const vector<T> &vec) const; // compute the error norm between the exact
                                   // solution and the global best position
  void init(const I &swarm_id, const I &max_iter, const T &tol, const T &w,
            const T &c1, const T &c2, const I &num_particles, const Fun &fun,
            const I &D,
